...
void on_dot_clicked(GtkButton* button, gpointer data) {    //当小数点按下时
	gint i;
	if (have_result)
		clear_all();    //全部清除
	if (have_dot == FALSE)
...
int main(int argc, char* argv[]) {
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_clear_clicked),
			NULL);
	entry = gtk_entry_new();
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_num_clicked),
			NULL);
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_suan_clicked),
			(gpointer) 1);
	gtk_box_pack_start(GTK_BOX(hbox1), button, TRUE, TRUE, 5);
	...
	button = gtk_button_new_with_label(".");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_dot_clicked),
			NULL);
	gtk_box_pack_start(GTK_BOX(hbox4), button, TRUE, TRUE, 5);
	button = gtk_button_new_with_label("=");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_eq_clicked),
			NULL);
	gtk_box_pack_start(GTK_BOX(hbox4), button, TRUE, TRUE, 5);
	button = gtk_button_new_with_label("/");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_suan_clicked),
