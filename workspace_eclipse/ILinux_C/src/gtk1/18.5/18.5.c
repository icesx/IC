...
gint progress_timeout(gpointer data) {
	...
	if (pdata->activity_mode)
		...
		new_val = gtk_progress_bar_get_fraction(GTK_PROGRESS_BAR(pdata->pbar))
				+ 0.01;
		if (new_val > 1.0)
			new_val = 0.0;
		/* 设置进度条的新值 */
	...
	/* 这是一个timeout 函数，返回 TRUE ，这样它就能够继续被调用 */
	return TRUE;
}
...
int main(int argc, char *argv[]) {
	...
	gtk_table_attach(GTK_TABLE(table), check, 0, 1, 0, 1, GTK_EXPAND | GTK_FILL,
			GTK_EXPAND | GTK_FILL, 5, 5);
	g_signal_connect(G_OBJECT(check), "clicked", G_CALLBACK(toggle_show_text),
			pdata);
	gtk_widget_show(check);
	/* 添加一个复选按钮，切换活动状态 */
	check = gtk_check_button_new_with_label("Activity mode");
	gtk_table_attach(GTK_TABLE(table), check, 0, 1, 1, 2, GTK_EXPAND | GTK_FILL,
			GTK_EXPAND | GTK_FILL, 5, 5);
	...
	gtk_table_attach(GTK_TABLE(table), check, 0, 1, 2, 3, GTK_EXPAND | GTK_FILL,
			GTK_EXPAND | GTK_FILL, 5, 5);
	g_signal_connect(G_OBJECT(check), "clicked", G_CALLBACK(toggle_orientation),
			pdata);
	gtk_widget_show(check);
	/* 添加一个按钮，用来退出应用程序 */
