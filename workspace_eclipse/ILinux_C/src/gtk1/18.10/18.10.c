...
void calendar_date_to_string(CalendarData *data, char *buffer, gint buff_len) {
	...
	gtk_calendar_get_date(GTK_CALENDAR(data->window), &tm.tm_year, &tm.tm_mon,
			&tm.tm_mday);
...
void calendar_select_font(GtkWidget *button, CalendarData *calendar) {
	...
	if (!calendar->font_dialog) {
		...
		g_signal_connect(G_OBJECT(window), "destroy",
				G_CALLBACK(gtk_widget_destroyed), &calendar->font_dialog);
		...
		g_signal_connect_swapped(
				G_OBJECT(GTK_FONT_SELECTION_DIALOG(window)->cancel_button),
...
void create_calendar() {
	...
	struct {
		char *label;
	} flags[] = { { "Show Heading" }, { "Show Day Names" },
			{ "No Month Change" }, { "Show Week Numbers" }, {
					"Week Start Monday" } };
	...
	g_signal_connect(G_OBJECT(window), "destroy", G_CALLBACK(gtk_main_quit),
			NULL);
	g_signal_connect(G_OBJECT(window), "delete-event", G_CALLBACK(gtk_false),
			NULL);
	...
	g_signal_connect(G_OBJECT(calendar), "month_changed",
			G_CALLBACK(calendar_month_changed), &calendar_data);
	g_signal_connect(G_OBJECT(calendar), "day_selected",
			G_CALLBACK(calendar_day_selected), &calendar_data);
	...
	g_signal_connect(G_OBJECT(calendar), "prev_month",
			G_CALLBACK(calendar_prev_month), &calendar_data);
	g_signal_connect(G_OBJECT(calendar), "next_month",
			G_CALLBACK(calendar_next_month), &calendar_data);
	g_signal_connect(G_OBJECT(calendar), "prev_year",
			G_CALLBACK(calendar_prev_year), &calendar_data);
	g_signal_connect(G_OBJECT(calendar), "next_year",
			G_CALLBACK(calendar_next_year), &calendar_data);
	...
	for (i = 0; i < 5; i++) {
		...
		g_signal_connect(G_OBJECT(toggle), "toggled",
				G_CALLBACK(calendar_toggle_flag), &calendar_data);
	...
	g_signal_connect(G_OBJECT(button), "clicked",
			G_CALLBACK(calendar_select_font), &calendar_data);
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(gtk_main_quit),
			NULL);
