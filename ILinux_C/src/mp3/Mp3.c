...
int main(int argc, char *argv[]) {
	...
	g_signal_connect(seek_scale, "value-changed",
			G_CALLBACK(seek_value_changed), NULL);
...
void gui_update_time(const gchar *time, const gint64 position,
		const gint64 length) {
...
static gboolean build_gstreamer_pipeline(const gchar *uri) {
	...
	gst_bus_add_watch(gst_pipeline_get_bus(GST_PIPELINE(play)), bus_callback,
			NULL);
...
gboolean play_file() {
	...
	if (play) {
		 ...
		 * playing */
		timeout_source = g_timeout_add(200, (GSourceFunc) update_time_callback,
				play);
		return TRUE;
	}
...
void seek_to(gdouble percentage) {
	...
	if (play && gst_element_query_duration(play, &fmt, &length)) {
		...
		if (!gst_element_seek(play, 1.0, GST_FORMAT_TIME, GST_SEEK_FLAG_FLUSH,
				GST_SEEK_TYPE_SET, target, GST_SEEK_TYPE_NONE,
				GST_CLOCK_TIME_NONE))
