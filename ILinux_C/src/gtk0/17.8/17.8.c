...
int main(int argc, char*argv[]) {
	...
	/*滚动条的出现方式设为GTK_POLICY_AUTOMATIC将自动定是否需要出现滚动条而设为
	 GTK_POLICY_ALWAYS，将一直显示一个滚动条*/
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolled_window),
			GTK_POLICY_AUTOMATIC, GTK_POLICY_ALWAYS);
	/*对话框窗口内部包含一个vbox构件*/
	gtk_box_pack_start(GTK_BOX(GTK_DIALOG(window)->vbox), scrolled_window, TRUE,
			TRUE, 0);
	...
	for (i = 0; i < 10; i++)
		for (j = 0; j < 10; j++) {
			...
			gtk_table_attach_defaults(GTK_TABLE(table), button, i, i + 1, j,
					j + 1);
