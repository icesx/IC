...
GtkWidget*make_box(gboolean homogeneous, gint spacing, gboolean expand,
		gboolean fill, guint padding) {
	GtkWidget*box;
	GtkWidget*button;
	char padstr[80];
	/*以合适的homogeneous和spacing设置创建一个新的横向盒*/
...
int main(int argc, char*argv[]) {
	...
	gtk_init(&argc, &argv);
	if (argc != 2) {
		fprintf(stderr, "usage:packboxnum,wherenumis1,2,or3.\n");
		/*这个在对GTK进行收尾处理后以退出状态为1退出。*/
		exit(1);
	}
	which = atoi(argv[1]);
	/*创建窗口*/
	window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	/*你应该总是记住连接delete_event信号到主窗口。这对
	 *适当的直觉行为很重要*/
	...
	/*将标签组装到纵向盒（vboxbox1）里。记住加到纵向盒里的
	 *构件将依次一个放在另一个上面地组装。*/
	...
	break;
	case3:
	/*这个示范了用gtk_box_pack_end()来右对齐构
	 *件的能力。首先，我们像前面一样创建一个新盒。*/
	box2 = make_box(FALSE, 0, FALSE, FALSE, 0);
	/*创建将放在末端的标签。*/
	label = gtk_label_new("end");
	/*用gtk_box_pack_end()组装它，这样它被放到
	...
	/*当然，还有我们的主函数。*/
	gtk_main();
	/*当gtk_main_quit()被调用时控制权(Control)返回到
	 *这里，但当exit()被使用时并不会。*/
	return 0;
}
