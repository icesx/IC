...
void cb__size( GtkAdjustment *get, GtkAdjustment *set ) {
	/* 将示例调整对象的 size和 increment size设置为" Size"比例构件指定的值 */
	set->page_size = get->value;
	set->step_increment = get->value;
	/* 设置调整对象的值并使它引发一个 "changed" 信号，以重新配置所有已经连接到这个调整对象的构件*/
...
void create_range_controls( void )  {
	...
	/* _size值只对滚动条构件有区别，并且，实际上能取得的最高值是(upper - _size) 。*/
	...
	box2 = gtk_hbox_new (FALSE, 10);
	gtk_container_set_border_width (GTK_CONTAINER (box2), 10);
	/* 以及，最后一个水平比例构件用于调整滚动条的  size 。 */
	label = gtk_label_new ("Scrollbar  Size:");
	gtk_box_pack_start (GTK_BOX (box2), label, FALSE, FALSE, 0);
	gtk_widget_show (label);
	adj2 = gtk_adjustment_new (1.0, 1.0, 101.0, 1.0, 1.0, 0.0);
	g_signal_connect (G_OBJECT (adj2), "value_changed",  G_CALLBACK (cb__size), adj1);
	scale = gtk_hscale_new (GTK_ADJUSTMENT (adj2));
	gtk_scale_set_digits (GTK_SCALE (scale), 0);
	gtk_box_pack_start (GTK_BOX (box2), scale, TRUE, TRUE, 0);
	gtk_widget_show (scale);
