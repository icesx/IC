...
{     
	...
	for (i = 0; i < 10; i++) {
		...
		g_free (msg);     }         cell = gtk_cell_renderer_text_new ();
...
// 时，其parent就是这个容器。层次关系被确定要求，其parent 的parent...也
// 确定了。顶级窗口可以不要parent 。只是我的经验理解。
void insert_text (GtkTextBuffer *buffer)
{
	GtkTextIter iter;
	gtk_text_buffer_get_iter_at_offset (buffer, &iter, 0);
	gtk_text_buffer_insert (buffer, &iter,"From: pathfinder@nasa.gov\nTo: mom@nasa.gov\nSubject: Made it!\n\nWe just got in this morning. The weather has been\n"\
...
int main( int   argc,char *argv[] ) {
	GtkWidget *window;
	GtkWidget *vpaned;     GtkWidget *list;
	GtkWidget *text;     gtk_init (&argc, &argv);
